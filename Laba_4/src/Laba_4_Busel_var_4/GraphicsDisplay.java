...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(5.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {5, 5, 5, 5, 5, 5, 15, 5,10,5,10}, 0.0f); //24,6,12,6,6,6,12,6
    ...
    protected void paintMarkers(Graphics2D canvas)
    {
        ...
        for (int i = 0; i < graphicsData.length; i++)
        {
           ...
           //(int)(graphicsData[i][1]);
            int m =(int) Math.round(graphicsData[i][1]);
             if (m%2==1)
                canvas.setColor(Color.RED);
            else
                canvas.setColor(Color.BLACK);
